...
\begin{frame}{証拠：生のLLMは金融文書で崩れる}{ボトルネックはモデルではなく、データ準備}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \scriptsize
      \begin{itemize}\setlength\itemsep{2.4mm}
        \item \kw{ネスト表PDF}：前処理なし\kwbad{56\%}→専用パイプラインで\kwgood{76\%}。
        \item \kw{FinanceBench}：GPT-4-Turbo＋検索でも\kwbad{81\%}が誤答・回答拒否。
        \item \kw{Fin-RATE}：企業横断比較で\kwbad{23.91\%→1.27\%}へ崩壊。
...
\begin{frame}{一つの解：AI-Readyパイプライン}{raw文書を「読み解ける」状態へ変換する}
  ...
  \begin{tikzpicture}[font=\footnotesize,node distance=4mm]
    \node[fnode muted,text width=1.6cm] (raw) {\textbf{raw文書}\\PDF/XBRL/HTML};
    \node[fnode blue,right=of raw,text width=1.6cm] (parse) {\textbf{パース／}\\\textbf{正規化}};
    \node[fnode blue,right=of parse,text width=1.9cm] (struct) {\textbf{構造化}\\表・数値・脚注の\\意味を保持};
    \node[fnode blue,right=of struct,text width=1.6cm] (chunk) {\textbf{チャンク化}\\／索引};
    \node[fnode accent,right=of chunk,text width=1.7cm] (search) {\textbf{階層・グラフ}\\\textbf{検索}};
    \node[fnode dark,right=of search,text width=1.8cm] (agent) {\textbf{LLM／}\\\textbf{エージェント}};
    \node[fnode good,right=of agent,text width=1.7cm] (outcome) {\textbf{アウトカム}\\（リターン）};
    \draw[farr] (raw)--(parse);
...
\begin{frame}{高精度＝最善とは限らない}{コストのパレートフロンティアで選ぶ}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tabular}{@{}lcc@{}}
        \toprule
         & \textbf{\color{brandbad}反射型} & \textbf{\color{brandgood}階層型} \\
        \midrule
        F1 & 0.943 & 0.924 \\
      ...
      \vskip1mm
      {\scriptsize
       \textcolor{brandbad}{\textbf{反射型}}＝答えを\kwbad{自己検証して再試行}（高精度・高コスト）\\
       \textcolor{brandgood}{\textbf{階層型}}＝管理役が\kwgood{下位に分担}（僅差の精度・低コスト）\par}
      \vskip1.5mm
      \begin{insight}{100万文書規模なら}
        \footnotesize
        コスト差は\kwg{\$282{,}000}。精度だけで調達を決めると
        \kwbad{TCOを見誤る}。\kw{規模に応じた設計}が鍵。
      \end{insight}
    \end{column}
  \end{columns}
